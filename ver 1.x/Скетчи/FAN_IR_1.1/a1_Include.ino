...
#include <FRAM.h>         // NEW библиотека для работы с FRAM памятью (записывает положение Servo). Работает от библиотеки "microWire.h" от AlexGyver(версия 2.2)

//"microWire" ver2.2 УЖЕ включена в библиотеку "FRAM", поэтому отдельно не нужно скачивать и подключать её в код.
